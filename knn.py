...
class KNN(BinaryClassifier):
    ...
    def predict(self, X):
        ...
        if self.trY.size == 0:
            ...
            ### TODO: YOUR CODE HERE
            # trained data is stored in trX, trY
            # input X is vector of x coordinates -> need to predict Y (val)
            # need to: find calculate the distance to each entry in trX,
            #   select the K closest trX entries, and find sum of their Y
            dis = [linalg.norm(row - X) for row in self.trX]
            args = argsort(dis, axis = 0)
            for arg in args[:K]:
                val += self.trY[arg]
            ...
            args = argsort(dis, axis = 0)
            ind = 0
            while ind < len(args) and dis[args[ind]] <= eps:
                val += self.trY[args[ind]]
                ind += 1
            return val
