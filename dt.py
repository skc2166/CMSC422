    ...
    def trainDT(self, X, Y, maxDepth, used):
        ...
        if maxDepth <= 0 or len(util.uniq(Y)) <= 1:
            ...
            for d in range(D):
                ...
                error = N - (leftY.count(util.mode(leftY)) + rightY.count(util.mode(rightY)))    ### TODO: YOUR CODE HERE
            ...
            if bestFeature < 0:
                ...
                for i in range(N):
                    if X[i][bestFeature] < 0.5:
                        ...
                        ri.append(i)
                self.left.trainDT(X[li], Y[li], maxDepth - 1, used)
                self.right.trainDT(X[ri], Y[ri], maxDepth - 1, used)
