...
class AlwaysPredictMostFrequent(BinaryClassifier):
    ...
    def train(self, X, Y):
        ...
        ### TODO: YOUR CODE HERE
        self.mostFrequentClass = self.predict(Y)
...
class FirstFeatureClassifier(BinaryClassifier):
    ...
    def train(self, X, Y):
        ...
        cfp = 0
        cfn = 0
        for feat, exp in zip(X[:,0], Y):
            if feat > 0:
                cfp = cfp + 1 if exp > 0 else cfp - 1
            else:
                cfn = cfn + 1 if exp > 0 else cfn - 1
        self.classForPos = 1 if cfp >= 0 else -1
        self.classForNeg = 1 if cfn >= 0 else -1